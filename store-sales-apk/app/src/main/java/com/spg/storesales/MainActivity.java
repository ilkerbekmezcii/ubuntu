...
public final class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){
        ...
        render(store.loadReport());syncMonitor();if(store.monitorEnabled()&&credentials.hasCredentials())startMonitor();
        if(credentials.hasCredentials())refreshAsync();else status.setText("microsoft.env seçildiğinde rapor bağlanır.");
    }
    ...
    private View ui(){
        ...
        r.addView(txt("Microsoft Rapor",24,true,"#111827"));TextView sub=txt("Microsoft Partner Center • hızlı önbellekli görünüm",13,false,"#6B7280");sub.setPadding(0,dp(2),0,dp(14));r.addView(sub);
        ...
        refresh=button("Yenile");refresh.setOnClickListener(v->refreshAsync());r.addView(refresh,params());
        monitor=button("5 dk takip");monitor.setOnClickListener(v->toggleMonitor());LinearLayout.LayoutParams p=params();p.topMargin=dp(8);r.addView(monitor,p);
    ...
    private void render(ReportStore.Report x){
        ...
        if(x.updatedAt>0)status.setText("Son başarılı veri: "+DateFormat.getDateTimeInstance(DateFormat.SHORT,DateFormat.SHORT).format(new Date(x.updatedAt))+" • "+Math.max(0,x.appCount)+" uygulama");
    }

    private void refreshAsync(){
        if(refreshing||!credentials.hasCredentials())return;refreshing=true;refresh.setEnabled(false);status.setText("Microsoft verisi kontrol ediliyor • mevcut rapor ekranda kalır");
        exec.execute(()->{try{
            ReportRepository repo=new ReportRepository(credentials,store);
            ReportStore.Report rr=repo.refresh((d,t,phase)->runOnUiThread(()->status.setText(phase+": "+d+"/"+t+" • rapor tamamlanınca güncellenecek")));
    ...
    @Override protected void onActivityResult(int rq,int rc,Intent d){super.onActivityResult(rq,rc,d);if(rq!=REQ_ENV||rc!=RESULT_OK||d==null||d.getData()==null)return;Uri u=d.getData();try(InputStream in=getContentResolver().openInputStream(u)){credentials.saveEnv(read(in));Toast.makeText(this,"Microsoft kimlik bilgileri şifreli kaydedildi",Toast.LENGTH_SHORT).show();refreshAsync();}catch(Exception e){Toast.makeText(this,"Dosya alınamadı: "+safe(e),Toast.LENGTH_LONG).show();}}
    private void toggleMonitor(){boolean next=!store.monitorEnabled();if(next&&!credentials.hasCredentials()){Toast.makeText(this,"Önce microsoft.env seçin",Toast.LENGTH_LONG).show();chooseEnv();return;}if(next&&Build.VERSION.SDK_INT>=33&&checkSelfPermission(Manifest.permission.POST_NOTIFICATIONS)!=PackageManager.PERMISSION_GRANTED)requestPermissions(new String[]{Manifest.permission.POST_NOTIFICATIONS},REQ_NOTIF);store.setMonitorEnabled(next);if(next)startMonitor();else stopService(new Intent(this,SalesMonitorService.class));syncMonitor();}
    private void startMonitor(){Intent i=new Intent(this,SalesMonitorService.class);if(Build.VERSION.SDK_INT>=26)startForegroundService(i);else startService(i);}
