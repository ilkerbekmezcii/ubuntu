...
function allowed(raw) {
  try {
    const u = new URL(raw);
    return u.protocol === 'https:' && u.hostname.endsWith('.oaiusercontent.com');
  } catch {
    return false;
  }
}
async function get(raw, binary = false) {
  if (!allowed(raw)) throw new Error('invalid_download_url');
...
export default async function handler(req, res) {
  try {
    const play = one(req.query?.play);
    const jks = one(req.query?.jks);
    const sign = one(req.query?.sign);
    const github = one(req.query?.github);
    if (!play || !jks || !sign || !github) return res.status(400).json({ ok: false, error: 'missing_url' });

    const [playText, jksBytes, signText, githubText] = await Promise.all([
      get(play), get(jks, true), get(sign), get(github)
    ]);
    const p = parseEnv(playText);
    const senv = parseEnv(signText);
    const g = parseEnv(githubText);
    const sa = {
      type: p.GOOGLE_SERVICE_ACCOUNT_TYPE,
      project_id: p.GOOGLE_PROJECT_ID,
      private_key_id: p.GOOGLE_PRIVATE_KEY_ID,
      private_key: p.GOOGLE_PRIVATE_KEY,
      client_email: p.GOOGLE_CLIENT_EMAIL,
      client_id: p.GOOGLE_CLIENT_ID,
      auth_uri: p.GOOGLE_AUTH_URI,
      token_uri: p.GOOGLE_TOKEN_URI,
      auth_provider_x509_cert_url: p.GOOGLE_AUTH_PROVIDER_CERT_URL,
      client_x509_cert_url: p.GOOGLE_CLIENT_CERT_URL,
      universe_domain: p.GOOGLE_UNIVERSE_DOMAIN || 'googleapis.com'
    };
