...
export default async function handler(req, res) {
  try {
    ...
    const bump = app.bump ? `sed -i "s/versionCode 106/versionCode 109/; s/versionName '2.0.6'/versionName '2.0.9'/" app/build.gradle\n` : '';
    const script = `set +e
...
apply_signing '${key}'
if [ -f gradlew ]; then chmod +x gradlew; GRADLE='./gradlew'; else GRADLE="$(find '${base}/.gradle/wrapper/dists' -type f -path '*/bin/gradle' | head -n 1)"; test -x "$GRADLE"; fi
$GRADLE --no-daemon clean test bundleRelease
